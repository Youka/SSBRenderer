...
void ssb_set_target(ssb_renderer renderer, int width, int height, int has_alpha, int aligned){
    if(renderer)
        reinterpret_cast<Renderer*>(renderer)->set_target(width, height, has_alpha, aligned);
}
